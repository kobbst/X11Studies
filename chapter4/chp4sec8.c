 ...
 *  been created externally to this program.  That bitmap pattern
 *  is stored in this program.  The program converts that pattern
 *  to the X Window System image format and that image format is
 *  written onto the base window using two different sets of
...
int main(int argc, char *argv)
{
  ...
  pattern = XCreatePixmapFromBitmapData(mydisplay, mywindow,
			   ...
			   BlackPixel(mydisplay, screen_num), 1);
